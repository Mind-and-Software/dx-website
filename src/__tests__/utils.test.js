...
describe('utility functions', () => {
  it('getNewTagArray forms a new tag array based on the current tag values and a new selected tag value', () => {
    ...
    expect(getNewTagArray('ALL', ['VALUE1', 'VALUE2', 'VALUE3'])).toEqual([
      'ALL',
    ]);
    // Remove the selected value from the list
    expect(getNewTagArray('VALUE2', ['VALUE1', 'VALUE2', 'VALUE3'])).toEqual([
      'VALUE1',
      'VALUE3',
    ]);
  ...
  it('filterItems filters the items based on the search value and selected tags', () => {
    ...
    expect(filterItems(testArticleListData, 'Test Author')).toEqual([
      testArticleListData[1],
      testArticleListData[2],
      testArticleListData[3],
    ]);
    expect(filterItems(testArticleListData, 'Test Author 2')).toEqual([
      testArticleListData[1],
    ]);
    expect(filterItems(testArticleListData, '', ['TAG1'])).toEqual([
      testArticleListData[0],
      testArticleListData[1],
    ]);
    expect(filterItems(testArticleListData, '', ['TAG1', 'TAG2'])).toEqual([
      testArticleListData[0],
      testArticleListData[1],
    ]);
    expect(
      filterItems(testArticleListData, 'Test Article 1', ['TAG1'])
    ).toEqual([testArticleListData[0]]);
    expect(filterItems(testArticleListData, 'Test Description 2')).toEqual([
      testArticleListData[1],
    ]);
    expect(
      filterItems(testArticleListData, 'Test Description 2', ['TAG3'])
    ).toEqual([]);
  });
  ...
  it('getItemsForPage finds the correct items/articles for the current page number from the total amount of items', () => {
    ...
    expect(
      getItemsForPage(1, 3, testArticleListData, jest.fn(), false)
    ).toEqual(testArticleListData.slice(0, 3));
    expect(
      getItemsForPage(1, 2, testArticleListData, jest.fn(), false)
    ).toEqual(testArticleListData.slice(0, 2));
    expect(
      getItemsForPage(2, 2, testArticleListData, jest.fn(), false)
    ).toEqual(testArticleListData.slice(2, 4));
    ...
    expect(
      getItemsForPage(3, 2, testArticleListData, jest.fn(), false)
    ).toEqual(testArticleListData.slice(2, 4));
    expect(
      getItemsForPage(3, 0, testArticleListData, jest.fn(), false)
    ).toEqual([]);
  });
